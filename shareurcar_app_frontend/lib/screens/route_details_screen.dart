...
class _RouteDetailsScreenState extends State<RouteDetailsScreen> {
  ...
  String _limpiarDireccion(String dir) => dir.replaceAll(", Alicante", "").trim();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("Detalles de la ruta", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text("Información del viaje", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                      Container(
                        padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                        decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(20)),
                        child: Text("${widget.ruta['available_seats']} plazas libres", style: TextStyle(color: Colors.green.shade700, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                  SizedBox(height: 20),
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            Icon(Icons.radio_button_checked, color: Colors.blue.shade600, size: 20),
                            SizedBox(width: 15),
                            Expanded(child: Text(_limpiarDireccion(widget.ruta['origin'].toString()), style: TextStyle(fontSize: 16))),
                            Text(_formatearHora(widget.ruta['departure_time'].toString()), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                          ],
                        ),
                        Padding(
                          padding: EdgeInsets.only(left: 9),
                          child: Align(alignment: Alignment.centerLeft, child: Container(height: 30, width: 2, color: Colors.grey.shade300)),
                        ),
                        Row(
                          children: [
                            Icon(Icons.location_on, color: Colors.red, size: 20),
                            SizedBox(width: 15),
                            Expanded(child: Text(_limpiarDireccion(widget.ruta['destination'].toString()), style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
                            Text(widget.ruta['arrival_time'] != null ? _formatearHora(widget.ruta['arrival_time'].toString()) : "--:--", style: TextStyle(fontSize: 16, color: Colors.grey)),
                          ],
                        ),
                      ],
                    ),
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 10),
                      Text("Frecuencia: ${widget.ruta['frequency'] ?? 'Puntual'}", style: TextStyle(fontSize: 16, color: Colors.grey.shade800)),
                    ],
                  ),
